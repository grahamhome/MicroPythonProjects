        ...
        while 1:
            if self._moving != self.should_move:
                if self._moving:
                    ...
                    self._moving = self.should_move
                    Timer(-1).init(
                        period=self._start_delay_sec * 1000,
                        mode=Timer.ONE_SHOT,
                        callback=self._go,
                    )
...
class Intersection:
    def __init__(
        ...
        self._traffic_light = TrafficLight(
            ...
            yellow_led_pin,
            green_led_pin,
            traffic_yellow_duration_sec,
            crosswalk_yellow_duration_sec,
            moving=False,
        )
        self._cars_waiting = 0
        self._crosswalk_light = CrosswalkLight(
            green_led_2_pin,
            yellow_led_2_pin,
            buzzer_pin,
            crosswalk_yellow_duration_sec,
